...
void initScreen(SSD1306_t *screen){
    //Init de l'écran
    i2c_master_init(screen, 21, 22, 0);
    screen->_flip = true;
    ssd1306_init(screen, 128, 64);
    ssd1306_clear_screen(screen, false);
    ssd1306_contrast(screen, 0xff);
    ssd1306_display_text_x2(screen, 0, "RUBIKS", 6, false);
	ssd1306_display_text_x2(screen, 5, "SOLVER", 6, false);   
	_ssd1306_line(screen,0,25,120,25,0);
	ssd1306_show_buffer(screen);
	_ssd1306_line(screen,0,26,120,26,0);
    ssd1306_show_buffer(screen);
    
}
void updatePageIHM(SSD1306_t *screen,uint8_t pageIHM){
	ssd1306_clear_screen(screen,0);
	switch (pageIHM){
		case 0:
			ssd1306_display_text_x2(screen, 1, " OPEN", 6, false);
			ssd1306_display_text_x2(screen, 4, "<    >", 6, false);			
		break;
		case 1:
			ssd1306_display_text_x2(screen, 1, " CLOSE", 6, false);
			ssd1306_display_text_x2(screen, 4, "<    >", 6, false);			
		break;
		case 2:
			ssd1306_display_text_x2(screen, 1, " HOME", 6, false);
			ssd1306_display_text_x2(screen, 4, "<    >", 6, false);			
		break;
		case 3:
			ssd1306_display_text_x2(screen, 1, " MIX", 6, false);
			ssd1306_display_text_x2(screen, 4, "<    >", 6, false);			
		break;
	}
}
